  ...
  useEffect(() => {
    async function loadTestimonials() {
      ...
      if (error) {
        ...
        const parsed = z.array(testimonialSchema).safeParse(formatted);
        if (parsed.success) {
          setTestimonials(parsed.data);
        } else {
          const valid = formatted.map(i => testimonialSchema.safeParse(i)).map(r => r.success ? r.data : null).filter(Boolean) as any as any;
